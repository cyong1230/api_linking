...
class DataReader:
    ...
    def read_matplotlib_all(self):
        matplotlib_ids = []
        f = open('matplotlib_qid.txt', 'r')
        for line in f:
            matplotlib_ids.append(line.strip())
        f.close()
        fw = open('matplotlib_q_a_c.txt', 'w')
        cnt = 1
        for id in matplotlib_ids:
            print cnt
            try: 
